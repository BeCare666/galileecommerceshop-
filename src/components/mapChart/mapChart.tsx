...
const MapWithCorridors: React.FC<MapChartProps> = ({ closeModal }, { setMapIsOk }: { setMapIsOk: (val: boolean) => void }) => {
    const router = useRouter();
    const chartRef = useRef<HTMLDivElement>(null);
    const [highlightCodes, setHighlightCodes] = useState<string[]>([]);
    const [selectedCorridors, setSelectedCorridors] = useState<any[] | null>(null);
        ...
        const fetchCorridors = async () => {
            try {
                ...
                fromCountryIdsNX.forEach((corridor: any) => {
                    if (corridor.from_countries_code) countryCodes.add(corridor.from_countries_code);
                    if (corridor.to_countries_code) countryCodes.add(corridor.to_countries_code);
                });
                ...
                >
                    <div
                        style={{
                            ...
                            overflowY: "auto",
                            borderRadius: 2,
                            boxShadow: "0 20px 50px rgba(0, 123, 255, 0.5), 0 0 10px #3b82f6aa inset",
                            color: "#e0e7ff",
                        }}
                        ...
                        {selectedCorridors.map((c, index) => (
                            <div
                                ...
                                onMouseLeave={(e) => {
                                    ...
                                    e.currentTarget.style.boxShadow = "0 6px 20px rgba(56, 189, 248, 0.2)";
                                }}
                            >
                                {/* Titre avec drapeaux */}
                                <div
                                    ...
                                    {[
                                        ...
                                        >
                                            ...
                                            <div style={{ display: "flex", flexDirection: "column" }}>
                                                <span style={{ color: "#cbd5e1", fontWeight: 600, fontSize: "14px" }}>{item.title} :</span>
                                                <span style={{ color: "#94a3b8", fontSize: "13px", marginTop: "2px" }}>{item.value}</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
